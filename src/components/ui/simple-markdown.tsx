    ...
    <div className={`markdown-content leading-relaxed ${className}`}>
      <ReactMarkdown
        components={{
          ...
          ul: ({ children }) => (
            <ul className="mb-3 pl-6 space-y-1 list-disc">{children}</ul>
          ),
          // Style ordered lists
          ol: ({ children }) => (
            <ol className="mb-3 pl-6 space-y-1 list-decimal">{children}</ol>
          ),
          // Style list items
          li: ({ children }) => <li className="leading-relaxed">{children}</li>,
          // Style headings
          h1: ({ children }) => (
            <h1 className="text-lg font-bold mb-3 mt-4 first:mt-0 text-slate-900">
              {children}
            </h1>
          ),
          h2: ({ children }) => (
            <h2 className="text-base font-bold mb-2 mt-3 first:mt-0 text-slate-900">
              {children}
            </h2>
          ),
          h3: ({ children }) => (
            <h3 className="text-sm font-bold mb-2 mt-2 first:mt-0 text-slate-900">
              {children}
            </h3>
          ),
          // Style bold text
